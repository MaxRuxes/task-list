...
namespace TaskList.ViewModel
{
    public class NonUrgentImportantTasksVM : ObservableObject
    {
        ...
        private string _fileName = @"\NonUrgentImportantTasks.xml", _fileDeletesName = @"\NonUrgentImportantTasks(Deletes).txt";
            ...
            get
            {
                return new DelegateCommand((o) =>
                {
                    ...
                    if (temp.ShowDialog() == true)
                    {
                        ...
                        {
                            File.AppendAllText(_filePath + @"\logs.txt", new string('.', 25) + "\tВАЖНОЕ И НЕСРОЧНОЕ ДЕЛО" + new string('.', 25) + Environment.NewLine);
