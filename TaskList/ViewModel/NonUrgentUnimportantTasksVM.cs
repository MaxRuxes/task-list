...
namespace TaskList.ViewModel
{
    public class NonUrgentUnimportantTasksVM : ObservableObject
    {
        ...
        private string _fileName = @"\NonUrgentUnimportantTasks.xml", _fileDeletesName = @"\NonUrgentUnimportantTasks(Deletes).txt";
            ...
            get
            {
                return new DelegateCommand((o) =>
                {
                    ...
                    if (temp.ShowDialog() == true)
                    {
                        ...
                        {
                            File.AppendAllText(_filePath + @"\logs.txt", new string('.', 25) + "\tНЕВАЖНОЕ И НЕСРОЧНОЕ ДЕЛО" + new string('.', 25) + Environment.NewLine);
